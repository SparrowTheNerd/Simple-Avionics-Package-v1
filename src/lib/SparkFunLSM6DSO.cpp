...
// status_t LSM6DSOCore::readRegisterInt32(int32_t* outputPointer, uint8_t address)
// {
//   uint8_t myBuffer[4];
// 	status_t returnError = readMultipleRegisters(myBuffer, address, 4);  //Does memory transfer
// 	int16_t output = myBuffer[0] | static_cast<uint16_t>(myBuffer[1] << 8);
...
LSM6DSO::LSM6DSO() 
{
	...
	imuSettings.gyroSampleRate = 416;   //Hz.  Can be: 13, 26, 52, 104, 208, 416, 833, 1666, 3332, 6664
	...
	imuSettings.accelRange = 8;      //Max G force readable.  Can be: 4, 8, 16, 32
	imuSettings.accelSampleRate = 416;  //Hz.  Can be: 1.6 (16), 12.5 (125), 26, 52, 104, 208, 416, 833, 1666, 3332, 6664
    ...
    case 32:
      ...
      break;
		...
		case 833:
			...
			break;
		case 1666:
			dataToWrite |= ODR_XL_1666Hz;
			break;
		case 3332:
			dataToWrite |= ODR_XL_3332Hz;
			break;
		case 6664:
			dataToWrite |= ODR_XL_6664Hz;
			break;
		}
	}
  ...
  // Write Accelerometer Settings....
	...
	if ( imuSettings.gyroEnabled == 1) {
		...
		case 833:
			...
			break;
		case 1666:
			dataToWrite |= ODR_GYRO_1666Hz;
			break;
		case 3332:
			dataToWrite |= ODR_GYRO_3332Hz;
			break;
		case 6664:
			dataToWrite |= ODR_GYRO_6664Hz;
			break;
		}
	}
...
bool LSM6DSO::setAccelDataRate(uint16_t rate) {
  ...
  switch ( rate ) {
    ...
    case 833:
      ...
      break;
    case 1666:
      regVal |= ODR_XL_1666Hz;
      break;
    case 3332:
      regVal |= ODR_XL_3332Hz;
      break;
    case 6664:
      regVal |= ODR_XL_6664Hz;
      break;
    default:
      break;
  }
...
float LSM6DSO::calcAccel( int16_t input )
{
  ...
  if( scale == 0 ) {
    switch( accelRange ){
      case 0:// Register value 0: 4g
        output = (static_cast<float>(input) * (.122)) / 1000;
        break;
      case 1: //Register value 1 : 32g
        output = (static_cast<float>(input) * (.976)) / 1000;
        break;
      case 2: //Register value 2 : 8g
        output = (static_cast<float>(input) * (.244)) / 1000;
        break;
      case 3://Register value 3: 16g
  ...
  if( scale == 1 ){
    switch( accelRange ){
      case 0: //Register value 0: 4g
        output = (static_cast<float>(input) * (0.122)) / 1000;
        break;
      case 1://Register value 1: 4g
        output = (static_cast<float>(input) * (0.122)) / 1000;
        break;
      case 2://Register value 2: 8g
        output = (static_cast<float>(input) * (.244)) / 1000;
...
bool LSM6DSO::setGyroDataRate(uint16_t rate) {
  ...
  switch( rate ) {
    ...
    case 833:
      ...
      break;
    case 1666:
      regVal |= ODR_GYRO_1666Hz;
      break;
    case 3332:
      regVal |= ODR_GYRO_3332Hz;
      break;
    case 6664:
      regVal |= ODR_GYRO_6664Hz;
      break;
    default:
      break;
  }
...
float LSM6DSO::getGyroDataRate(){
  ...
  switch( regVal ){
    ...
    case ODR_GYRO_833Hz:
      return 833.0;
    case ODR_GYRO_1666Hz:
      return 1666.0;
    case ODR_GYRO_3332Hz:
      return 3332.0;
    case ODR_GYRO_6664Hz:
      return 6664.0;
    default:
      return static_cast<float>(IMU_GENERIC_ERROR);
  }
...
fifoData LSM6DSO::fifoRead() {
  ...
  //   // uint8_t myBuffer[4];
  //   // status_t returnError = readMultipleRegisters(myBuffer, FIFO_DATA_OUT_X_L, 4);  //Does memory transfer
  //   // int32_t output = ((myBuffer[0] | static_cast<uint32_t>(myBuffer[1] << 8)) | static_cast<uint32_t>(myBuffer[2] << 16)) | static_cast<uint32_t>(myBuffer[3] << 32);
  //   // tempFifoData.timeStamp = output;

  //   int16_t data1;
  //   int16_t data2;
  //   readRegisterInt16(&data1, FIFO_DATA_OUT_X_L);
  //   readRegisterInt16(&data2, FIFO_DATA_OUT_Y_L);
  //   uint16_t output = data1;//(data1 << 16) | data2;
  //   tempFifoData.timeStamp = output;
